...
export async function getDueCardsForChat(
  chatSessionId: string
): Promise<FlashcardItem[]> {
  const query = `?${new URLSearchParams({ chat_session_id: chatSessionId }).toString()}`;
  return authFetch(apiUrl(`/flashcards/due${query}`));
}
